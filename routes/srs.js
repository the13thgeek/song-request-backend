...
router.post('/init-game', (req, res) => {
    ...
    try {
        ...
        console.log(`Game [${gameData.game_title} (${gameData.songs.length} songs)] initialized. Requests are currently ${requestsOpen ? "ON" : "OFF"}.`);
        statusRelay();
        res.status(200).json({
            status: true,
            message: `Game [${gameData.game_title} (${gameData.songs.length} songs)] initialized.`,
            id: gameData.game_id,
            title: gameData.game_title,
            year: gameData.game_year,
            song_count: gameData.songs.length,
